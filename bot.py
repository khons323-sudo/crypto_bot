"""
bot.py  ─  코인·유가 텔레그램 모니터링 봇 (단독 실행 버전)
실행: python bot.py
"""
...
# ── 알림 임계값 (봇 실행 중 /setalert 로 변경 가능) ──
cfg = {
...
def fetch_prices() -> dict:
    """Binance 무료 API - 코인 가격"""
    try:
        r = requests.get(
            "https://api.binance.com/api/v3/ticker/24hr",
            timeout=10
        ).json()
        d = {x["symbol"]: x for x in r}
        result = {}
        for name, sym in [("BTC","BTCUSDT"),("DOGE","DOGEUSDT"),
                           ("ETH","ETHUSDT"),("SOL","SOLUSDT")]:
            if sym in d:
                result[name] = {
                    "price": float(d[sym]["lastPrice"]),
                    "chg":   float(d[sym]["priceChangePercent"]),
                    "high":  float(d[sym]["highPrice"]),
                    "low":   float(d[sym]["lowPrice"]),
                    "vol":   float(d[sym]["quoteVolume"]),
                }
        return result
...
def fmt_coin(p, sym):
    return f"${p:,.4f}" if sym == "DOGE" else f"${p:,.0f}"
...
def build_full_brief() -> str:
    ...
    def cline(name):
        d = c.get(name)
        if not d or "_err" in c: return f"  {name}: 오류"
        a = arrow(d["chg"])
...
def check_alerts(c: dict, o: dict, fg: dict, ls: dict) -> list[str]:
    alerts = []
    btc  = c.get("BTC",  {})
    doge = c.get("DOGE", {})
    wti  = o.get("WTI",  {})
    ...
    if btc.get("price", 0) >= cfg["btc_high"] and cd_ok("btc_high"):
        ...
        alerts.append(
            f"🚀 *BTC 상단 돌파!*\n"
            f"${p:,.0f} (≥ ${cfg['btc_high']:,.0f})\n"
            f"다음 저항: $70,906 → $74,100\n"
            f"변동: {btc['chg']:+.2f}%"
        )
    elif btc.get("price", 99999) <= cfg["btc_low"] and cd_ok("btc_low"):
        p = btc["price"]
        alerts.append(
            f"⚠️ *BTC 하단 이탈!*\n"
            f"${p:,.0f} (≤ ${cfg['btc_low']:,.0f})\n"
    ...
    elif doge.get("price", 1) <= cfg["doge_low"] and cd_ok("doge_low"):
        alerts.append(
            f"🐕🔴 *DOGE $0.08 이탈!*\n"
            f"${doge['price']:.4f}\n"
            f"심층 하락 경고"
...
async def cmd_start(u: Update, _):
    await u.message.reply_text(
        ...
        "/fg    공포탐욕지수\n"
        "/alert 알림 임계값 확인\n"
        "/setalert KEY VALUE  (임계값 변경)\n\n"
        "⏰ 자동 알림: 임계값 돌파 즉시 + 1시간마다 브리핑",
...
async def cmd_alert(u: Update, _):
    await u.message.reply_text(
        ...
        f"변경 예시:\n"
        f"`/setalert BTC_HIGH 72000`\n"
        f"`/setalert DOGE_LOW 0.075`",
        parse_mode=ParseMode.MARKDOWN
    )
...
def main():
    if not BOT_TOKEN:
        print("❌ .env 파일에 TELEGRAM_BOT_TOKEN 을 입력하세요")
        return
    if not CHAT_ID:
        print("❌ .env 파일에 TELEGRAM_CHAT_ID 를 입력하세요")
        return
